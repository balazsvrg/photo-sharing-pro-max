...
@api_view(['GET', 'POST'])
@permission_classes([AllowAny])  # Default to AllowAny for GET
def photo_list_create(request):
    if request.method == 'GET':
        # Allow any user to access the list of photos
        photos = Photo.objects.all().order_by('upload_date')
        serializer = PhotoSerializer(photos, many=True)
        return Response(serializer.data)
    
    elif request.method == 'POST':
        # Only authenticated users can create a photo
        permission_classes = [IsAuthenticated]  # Change permission for POST method
